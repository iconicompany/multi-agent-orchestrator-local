...
import { Classifier, ClassifierResult } from "./classifiers/classifier";
...
export class MultiAgentOrchestrator {
  ...
  constructor(options: OrchestratorOptions = {}) {
    ...
    this.config = {
      LOG_AGENT_CHAT: options.config?.LOG_AGENT_CHAT ?? DEFAULT_CONFIG.LOG_AGENT_CHAT,
      LOG_CLASSIFIER_CHAT: options.config?.LOG_CLASSIFIER_CHAT ?? DEFAULT_CONFIG.LOG_CLASSIFIER_CHAT,
      LOG_CLASSIFIER_RAW_OUTPUT: options.config?.LOG_CLASSIFIER_RAW_OUTPUT ?? DEFAULT_CONFIG.LOG_CLASSIFIER_RAW_OUTPUT,
      LOG_CLASSIFIER_OUTPUT: options.config?.LOG_CLASSIFIER_OUTPUT ?? DEFAULT_CONFIG.LOG_CLASSIFIER_OUTPUT,
      LOG_EXECUTION_TIMES: options.config?.LOG_EXECUTION_TIMES ?? DEFAULT_CONFIG.LOG_EXECUTION_TIMES,
      MAX_RETRIES: options.config?.MAX_RETRIES ?? DEFAULT_CONFIG.MAX_RETRIES,
      MAX_MESSAGE_PAIRS_PER_AGENT: options.config?.MAX_MESSAGE_PAIRS_PER_AGENT ?? DEFAULT_CONFIG.MAX_MESSAGE_PAIRS_PER_AGENT,
      USE_DEFAULT_AGENT_IF_NONE_IDENTIFIED:
        options.config?.USE_DEFAULT_AGENT_IF_NONE_IDENTIFIED ?? DEFAULT_CONFIG.USE_DEFAULT_AGENT_IF_NONE_IDENTIFIED,
      CLASSIFICATION_ERROR_MESSAGE: options.config?.CLASSIFICATION_ERROR_MESSAGE,
      NO_SELECTED_AGENT_MESSAGE: options.config?.NO_SELECTED_AGENT_MESSAGE ?? DEFAULT_CONFIG.NO_SELECTED_AGENT_MESSAGE,
  ...
  getAllAgents(): { [key: string]: { name: string; description: string } } {
    return Object.fromEntries(Object.entries(this.agents).map(([key, { name, description }]) => [key, { name, description }]));
  }
  ...
  async dispatchToAgent(params: DispatchToAgentsParams): Promise<any | AsyncIterable<any>> {
    ...
    try {
      if (!classifierResult.selectedAgent) {
        ...
        const response = await this.measureExecutionTime(`Agent ${selectedAgent.name} | Processing request`, () =>
          selectedAgent.processRequest(userInput, userId, sessionId, agentChatHistory, additionalParams),
        );
        ...
        if (response.context && Object.keys(response.context).length) {
          await this.storage.saveContext(userId, sessionId, response.context);
        }
  ...
  async classifyRequest(userInput: string, userId: string, sessionId: string): Promise<ClassifierResult> {
    try {
      ...
      const classifierResult = await this.measureExecutionTime("Classifying user intent", () =>
  ...
  async agentProcessRequest(
    ...
    try {
      ...
      if (this.isAsyncIterable(agentResponse)) {
        ...
        this.processStreamInBackground(agentResponse, accumulatorTransform, userInput, userId, sessionId, classifierResult.selectedAgent);
        return {
          metadata,
  ...
  async routeRequest(userInput: string, userId: string, sessionId: string, additionalParams: Record<any, any> = {}): Promise<AgentResponse | any> {
    this.executionTimes = new Map();
  ...
  private async processStreamInBackground(
    ...
    try {
      ...
      if (fullResponse) {
        if (agent.saveChat) {
          await saveConversationExchange(userInput, fullResponse, this.storage, userId, sessionId, agent.id);
        }
      } else {
  ...
  private measureExecutionTime<T>(timerName: string, fn: () => Promise<T> | T): Promise<T> {
