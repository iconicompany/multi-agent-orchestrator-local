...
import { ANTHROPIC_MODEL_ID_CLAUDE_3_5_SONNET, ConversationMessage, ParticipantRole, TemplateVariables } from "../types";
...
export class AnthropicAgent extends Agent {
   ...
   */
  private isAgentTools(tool: AgentTools | Anthropic.Tool[]): tool is AgentTools {
   ...
   */
  private formatToolResults(toolResults: AgentToolResult[] | any): ConversationMessage {
  ...
  async processRequest(
    ...
    const messages: Anthropic.MessageParam[] = chatHistory.map((message) => ({
      role: message.role === ParticipantRole.USER ? ParticipantRole.USER : ParticipantRole.ASSISTANT,
      ...
      if (this.streaming) {
        ...
        do {
          ...
          const llmInput = {
            ...
            ...(this.toolConfig && {
              tools: this.toolConfig.tool instanceof AgentTools ? this.formatTools(this.toolConfig.tool) : this.toolConfig.tool,
            }),
          };
          ...
          if (toolUseBlocks.length > 0) {
            ...
            const textContent = response.content.find((content): content is Anthropic.TextBlock => content.type === "text");
